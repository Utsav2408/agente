...
def show_chat():
    ...
    for sender, msg, ts in st.session_state['chat_history']:
        ...
        style = "background:#e0f7fa;text-align:right;" if sender=='user' else "background:#f1f8e9;text-align:left;"
        st.markdown(
            f"<div style='{style} padding:10px;border-radius:10px;margin:10px 0;'><strong>{sender.title()}:</strong> {msg}{time_div}</div>",
